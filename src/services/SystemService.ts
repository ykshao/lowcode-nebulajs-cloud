...
export  class SystemService {
    ...
    static async initDatabase(){
        ...
        if(needInitialize){
            ...
            // 初始化数据
            const sqlFile = fs.readFileSync(path.resolve('./res/init.sql')).toString()
            const sqlCmds = sqlFile.split(/;\n/).map(s => s.trim()).filter(s => s)
        ...
        // const [results1] = await nebula.sequelize.query("select * from app_user")
